...
st.markdown("""
<style>
    .main-header { font-size: 2.2rem; font-weight: 700; color: #1a1a2e; text-align: center; padding: 0.5rem 0; }
    .sub-header { font-size: 1.0rem; color: #666; text-align: center; margin-bottom: 1.5rem; }
</style>
...
def load_model_metrics():
    ...
    if os.path.exists(path):
        try:
            ...
            pass
    for p in ['models/gradient_boosting_model.pkl', 'models/demand_forecaster.pkl']:
        if os.path.exists(p):
            try:
                import pickle
                with open(p, 'rb') as f:
...
if df is None:
    st.error("Data not found! Please check data folder.")
    st.stop()
...
if data_source and 'ml_ready_data' in data_source:
    st.sidebar.success(f"🟢 FULL dataset: {len(df):,} rows")
else:
    st.sidebar.info(f"☁️ Cloud dataset: {len(df):,} rows")
...
# Apply filters
filtered_df = df[(df['store_id'].isin(selected_stores)) & (df['date'].dt.year.isin(selected_years))]
...
if page == "📊 Executive Overview":
    ...
    st.markdown('<div class="sub-header">Tajir Retail — Kiryana Store Demand Forecasting & Inventory Optimization</div>', unsafe_allow_html=True)

    total_revenue = float(filtered_df['sales'].sum())
    avg_daily = float(filtered_df['sales'].mean())
    n_stores = int(filtered_df['store_id'].nunique())
    n_products = int(filtered_df['family'].nunique())
    stockout_rate = float(filtered_df['is_zero_sale'].mean() * 100) if 'is_zero_sale' in filtered_df.columns else 0.0

    col1, col2, col3, col4, col5 = st.columns(5)
    col1.metric("💰 Total Revenue", f"${total_revenue:,.0f}")
    col2.metric("📈 Avg Daily Sales", f"${avg_daily:,.1f}")
    col3.metric("🏪 Active Stores", str(n_stores))
    col4.metric("📦 Product Families", str(n_products))
    ...
    with col_right:
        ...
        if 'store_type' in filtered_df.columns:
            ...
            fig = px.bar(by_type, x='Store Type', y='Total Sales', color='Avg Sales', color_continuous_scale='Viridis')
    ...
    with col_left2:
        ...
        fig = px.bar(top_prod, x='sales', y='family', orientation='h', color='sales', color_continuous_scale='Blues')
    ...
    season_cols = {
        'is_ramadan_period': 'Ramadan', 'is_wedding_season': 'Wedding Season',
        'is_summer_peak': 'Summer Peak', 'is_payday_week': 'Payday Week',
    ...
    for col, name in season_cols.items():
        if col in filtered_df.columns:
            ...
            season_data.append({'Season': name, 'During Season': round(in_s, 1), 'Normal Period': round(out_s, 1), 'Impact (%)': round(impact, 1)})

    if season_data:
        season_df = pd.DataFrame(season_data)
        col_s1, col_s2 = st.columns(2)
        with col_s1:
            fig = px.bar(season_df, x='Season', y=['During Season', 'Normal Period'], barmode='group', color_discrete_sequence=['#E74C3C', '#3498DB'])
            fig.update_layout(height=400, yaxis_title='Avg Daily Sales')
            st.plotly_chart(fig, use_container_width=True)
        with col_s2:
            colors = ['#E74C3C' if x > 0 else '#2ECC71' for x in season_df['Impact (%)']]
            fig = go.Figure(go.Bar(x=season_df['Impact (%)'], y=season_df['Season'], orientation='h', marker_color=colors))
...
# ============================================
    ...
    st.markdown('<div class="sub-header">Identifying where revenue is lost due to stockouts</div>', unsafe_allow_html=True)
    ...
    avg_selling = float(filtered_df[filtered_df['sales'] > 0]['sales'].mean()) if (filtered_df['sales'] > 0).any() else 0
    ...
    with col_left:
        ...
        if 'store_type' in filtered_df.columns and 'is_zero_sale' in filtered_df.columns:
            ...
            fig = px.bar(by_type, x='Store Type', y='Stockout Rate (%)', color='Stockout Rate (%)', color_continuous_scale='Reds')
            fig.update_layout(height=400)
            st.plotly_chart(fig, use_container_width=True)
    ...
    with col_right:
        ...
        if 'is_zero_sale' in filtered_df.columns:
            ...
            fig = px.bar(by_cat, x='Stockout Rate (%)', y='Category', orientation='h', color='Stockout Rate (%)', color_continuous_scale='OrRd')
            fig.update_layout(height=400)
            st.plotly_chart(fig, use_container_width=True)
    ...
    if 'is_zero_sale' in filtered_df.columns:
        ...
        fig = px.imshow(pivot.values, labels=dict(x="Category", y="Store ID", color="Stockout %"),
                       x=list(pivot.columns), y=[str(s) for s in pivot.index], color_continuous_scale='RdYlGn_r')
        fig.update_layout(height=500)
        st.plotly_chart(fig, use_container_width=True)

    if stockout_risk is not None:
        st.subheader("⚠️ Risk Assessment Table")
        display_cols = [c for c in ['store_id', 'family', 'city', 'stockout_rate', 'risk_score', 'risk_category', 'avg_sales'] if c in stockout_risk.columns]
...
# ============================================
    ...
    st.markdown('<div class="sub-header">Gradient Boosting model trained on 2.6M+ records</div>', unsafe_allow_html=True)

    col1, col2, col3, col4 = st.columns(4)
    if model_metrics:
        col1.metric("MAE", f"{float(model_metrics.get('MAE', 0)):,.2f}")
        col2.metric("RMSE", f"{float(model_metrics.get('RMSE', 0)):,.2f}")
        col3.metric("R² Score", f"{float(model_metrics.get('R2', 0)):.4f}")
        col4.metric("MAPE", f"{float(model_metrics.get('MAPE', 0)):.2f}%")
    else:
        col1.metric("MAE", "N/A")
    ...
    if predictions is not None and len(predictions) > 0:
        ...
        with col_left:
            ...
            fig.add_shape(type='line', x0=0, y0=0, x1=max_val, y1=max_val, line=dict(color='red', dash='dash', width=2))
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=timeline['Period'], y=timeline['Actual'], name='Actual', line=dict(color='#2E86C1', width=2)))
        fig.add_trace(go.Scatter(x=timeline['Period'], y=timeline['Predicted'], name='Predicted', line=dict(color='#E74C3C', width=2)))
    ...
    if feature_imp is not None and len(feature_imp) > 0:
        ...
        colors = ['#E74C3C' if any(k in str(f).lower() for k in pakistan_kw) else '#3498DB' for f in top_f['feature']]
        fig = go.Figure(go.Bar(x=top_f['importance'], y=top_f['feature'], orientation='h', marker_color=colors))
        fig.update_layout(title=f'Top {top_n} Features (Red = Pakistan-specific)', height=max(400, top_n * 25), xaxis_title='Importance')
        st.plotly_chart(fig, use_container_width=True)
...
# ============================================
    ...
    if alerts is not None and len(alerts) > 0:
        col1, col2, col3, col4 = st.columns(4)
        col1.metric("🔴 Critical", str(len(alerts[alerts['alert_level'] == 'CRITICAL'])))
        col2.metric("🟠 High", str(len(alerts[alerts['alert_level'] == 'HIGH'])))
        col3.metric("🟡 Medium", str(len(alerts[alerts['alert_level'] == 'MEDIUM'])))
        col4.metric("🟢 Low", str(len(alerts[alerts['alert_level'] == 'LOW'])))

        st.markdown("---")
        severity_filter = st.multiselect("Filter by Level", ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW'], default=['CRITICAL', 'HIGH', 'MEDIUM', 'LOW'])
        ...
        display_cols = [c for c in ['alert_level', 'store_id', 'city', 'family', 'avg_daily_demand', 'suggested_order', 'consecutive_zeros', 'alert_reason'] if c in filtered_alerts.columns]
        st.dataframe(filtered_alerts[display_cols], use_container_width=True, height=500)
        st.download_button("📥 Download Alerts CSV", filtered_alerts.to_csv(index=False), "reorder_alerts.csv", "text/csv")
    else:
        st.info("No alerts available.")

    if reorder_recs is not None and len(reorder_recs) > 0:
        st.markdown("---")
        st.subheader("📊 Reorder Recommendations")
        display_cols = [c for c in ['store_id', 'family', 'city', 'avg_daily_demand', 'reorder_point_95', 'safety_stock_95', 'suggested_order_qty', 'risk_category'] if c in reorder_recs.columns]
...
# ============================================
    ...
    if len(store_data) > 0:
        city = str(store_data['city'].iloc[0]) if 'city' in store_data.columns else 'N/A'
        stype = str(store_data['store_type'].iloc[0]) if 'store_type' in store_data.columns else 'N/A'
        rev = float(store_data['sales'].sum())
        avg = float(store_data['sales'].mean())
        so_rate = float(store_data['is_zero_sale'].mean() * 100) if 'is_zero_sale' in store_data.columns else 0.0
        ...
        with col_right:
            ...
            fig = px.bar(bp, x='Sales', y='Product', orientation='h', color='Sales', color_continuous_scale='Blues')
            fig.update_layout(height=400)
            st.plotly_chart(fig, use_container_width=True)

        st.subheader(f"🚨 Stockout by Product — Store {selected_store}")
        if 'is_zero_sale' in store_data.columns:
            by_fam = store_data.groupby('family').agg(total=('sales', 'count'), stockouts=('is_zero_sale', 'sum')).reset_index()
